...
optimization by invoking it only when deemed beneficial. In the
...
episode level. Across these diverse mechanisms, all methods share a common
assumption: that uncertainty signals have a fixed,
environment-independent relationship with compute value. This
assumption is seldom challenged because the vast majority of
adaptive compute research operates on homogeneous reasoning
benchmarks (MATH, GSM8K, GPQA), where signal direction happens
to be consistent. We show that the assumption is incorrect in
heterogeneous \emph{agent} environments: signal--utility direction
reverses across environments, and no fixed-direction strategy can
guarantee non-negative value of computation
(Proposition~\ref{prop:necessity}). A detailed per-method
comparison is provided in Appendix~\ref{app:related}.
