...
Bar chart comparing EAAG (with LLM features) vs principled_v2 (without LLM features). LLM contribution is marginal (<1pp) in most envs, except FEVER (+9.1pp).
...
- `results/phase6/path_e/{env}/se_online_decay_local/` -- EAAG (with LLM features)
