...
## 6.5 新环境结果 (Phase 6.1, 2026-03-20)

### FEVER — Fact Verification (search-based QA, 与 HotpotQA 同族)

| 类别 | 方法 | Mean SR | Ro/ep | 备注 |
|------|------|:-------:|:-----:|------|
| Ref | base_only | 37.0% | 0.00 | |
| Ref | always_trigger | ~99.8% | 1.46 | |
| Ref | oracle | ~99.8% | 0.71 | |
| **Ours** | **scg_finetune_lr** | **98.0%** | **0.99** | **Δ=+61pp, cost 节省 32%** |
| Ours | best_sigma_wrong | 63.0% | 4.31 | 错误方向 → −36.8pp |
| CB | s1_budget | 46.2% | 1.58 | SCG >> s1 (+51.8pp) |
| CB | auq | 40.7% | 1.17 | SCG >> auq (+57.3pp) |
| CB | catts/cats/seag/corefine | 33-35% | ~0 | ⚠️ 不触发 (Issue 3), 待重跑 |
| Path E | proto | 43.5% | 0.92 | 19/75 runs 完成 |
| Path E | principled_nopca | 40.7% | 1.11 | |

**Key**: SCG(98.0%) Pareto-dominates 所有已完成的 CB 和 Path E 方法。BSW 错误方向 −36.8pp 是 direction reversal 的直接证据。

### APPS Interview — Code Generation (Interview 难度, rollout-safe)

| 类别 | 方法 | Mean SR | Ro/ep | 备注 |
|------|------|:-------:|:-----:|------|
| Ref | base_only | 60.5% | 0.00 | |
| Ref | always_trigger | 79.5% | 2.19 | |
| Ref | oracle | 79.5% | 0.23 | |
| **Ours** | **scg_finetune_lr** | **79.5%** | **1.00** | = ceiling, cost 节省 54% |
| Ours | best_sigma_wrong | 79.5% | 1.39 | Rollout-safe: 错误方向也有效 |
| CB | s1_budget | 69.0% | 1.00 | SCG > s1 (+10.5pp) |
| CB | auq | 64.7% | 1.08 | SCG > auq (+14.8pp) |
| CB | catts/cats/seag/corefine | 60.5% | ~0 | ⚠️ 不触发 (Issue 3), 待重跑 |

**Key**: Rollout-safe 环境, 所有方法接近 79.5% ceiling。SCG 的优势仅在 cost。

### CRUXEval — Code Reasoning (output prediction, base=85%)

| 类别 | 方法 | Mean SR | Ro/ep | 备注 |
|------|------|:-------:|:-----:|------|
| Ref | base_only | 85.0% | 0.00 | 超过 GO ceiling |
| Ref | always_trigger | 99.5% | 1.91 | |
| **Ours** | **scg_finetune_lr** | **99.5%** | **0.90** | = ceiling, cost 节省 53% |
| Ours | best_sigma_wrong | 87.5% | 1.00 | 错误方向 −12pp |

**Key**: Rollout-safe, base=85% 偏高。SCG 和 always_trigger 都是 99.5%。

### DS-1000 — ❌ 数据暂不可用

修复 oracle leak 时引入新 bug (Issue 4, base_only=100%)，所有 rerun 数据不可用。
修复前可参考: base=20.3%, random_50=61.7%, scg=71.7% (这些是 gate 不触发时的真实数据)。
...
论文中建议展示 3 个代表性方法，体现 "online direction learning" 框架：

1. **SCG (scg_finetune_lr)** — 手工 feature baseline，展示领域知识的价值和局限
2. **Principled v2** — 全自动化 (LASSO + adaptive lambda + fallback)，无需领域知识，5 环境全覆盖
3. **SE (se_c3_addlat_local 或 se_few5_filter_local)** — LLM 自动 feature discovery

**SE 代表性方法选择:**

| 维度 | se_c3_addlat_local | se_few5_filter_local |
|------|:------------------:|:--------------------:|
| 5env 均值 SR | **65.7%** | 65.2% |
| 5env 均值 Cost | 3.17× | **3.12×** |
| WebShop | **43.2%** | 39.2% |
| APPS | 64.3% | **65.8%** |
| Plancraft | 27.3% | **28.0%** |
| 算法复杂度 | 3次 LLM 反思 | **1次 LLM 反思** |
| Backend | ✅ local | ✅ local |
| BabyAI 数据 | 无 | ✅ 有 (3.5%) |
| 论文叙事 | "Multi-Reflection" | "LLM Feature Discovery" |

> **建议**: 主表用 se_c3_addlat_local (5env SR 最高), 方法描述用 se_few5_filter_local (算法最简), 两者都报告作为消融对比。se_online_fix_local (WebShop 44.0% 但 Plancraft 退化) 作为 ε-greedy 消融展示。
