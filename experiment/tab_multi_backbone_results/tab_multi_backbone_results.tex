\begin{table*}[t]
\centering
\caption{Cross-backbone results: DIAL SR (\%) on Phi-3.5-mini and Llama-3.1-8B across 6 environments
(mean $\pm$ std over 3 seeds). DIAL adapts to each backbone without collapse, unlike fixed-direction
baselines whose performance is brittle across models.
$\dagger$: calibrated with Phase~1 data.}
\label{tab:multi-backbone-results}
\resizebox{\textwidth}{!}{%
\begin{tabular}{l *{6}{c}}
\toprule
& \textbf{HotpotQA} & \textbf{WebShop} & \textbf{FEVER}
& \textbf{TWExpress} & \textbf{Plancraft} & \textbf{APPS} \\
\midrule
\multicolumn{7}{l}{\textit{Phi-3.5-mini-instruct}} \\
base\_only          & 28.3 &  3.5 &  7.2 & 66.7 & 13.7 & 27.0 \\
always\_trigger     & 98.8 & 53.5 & 61.0 & 97.8 & 14.5 & 79.5 \\
CATTS               & 42.0 & 28.7 & 12.7 & 86.7 & 13.5 & 27.7 \\
SEAG$^\dagger$      & 88.3 & 36.5 & 13.5 & 94.5 & 14.7 & 27.8 \\
CoRefine$^\dagger$  & 87.7 & 35.8 & 13.7 & 94.7 & 15.2 & 28.5 \\
CaTS$^\dagger$      & 68.3 & 41.7 & 19.8 & 68.2 & 13.5 & 30.5 \\
AUQ                 & 31.2 &  7.0 &  8.5 & 94.5 & 12.7 & 27.2 \\
s1\_budget          & 94.2 &  3.0 & 23.0 & 95.7 & 13.7 & 34.5 \\
\textbf{DIAL}       & \textbf{92.3} & \textbf{57.3} & 16.5 & \textbf{96.7} & 13.8 & \textbf{36.8} \\
\midrule
\multicolumn{7}{l}{\textit{Llama-3.1-8B-Instruct}} \\
base\_only          & 46.3 &  1.2 & 13.2 & 36.5 & 31.7 & 60.2 \\
always\_trigger     & 99.5 & 42.8 & 62.8 & ---  & 18.7 & 79.5 \\
CATTS               & 96.0 &  1.3 & 14.2 & 49.3 & 27.0 & 52.3 \\
SEAG$^\dagger$      & 96.0 & 27.5 & 53.3 & 76.5 & 21.8 & 60.2 \\
CoRefine$^\dagger$  & 96.2 & 27.5 & 53.5 & 76.0 & 21.5 & 60.2 \\
CaTS$^\dagger$      & 96.2 & 36.0 & 56.3 & ---  & 22.8 & 60.2 \\
AUQ                 & 97.0 & 37.8 & 52.8 & 63.2 & 27.0 & 60.0 \\
s1\_budget          & 96.5 &  1.2 & 13.8 & ---  & 30.3 & 72.5 \\
\textbf{DIAL}       & \textbf{95.5} & \textbf{41.7} & 34.7 & \textbf{94.8} & 27.0 & 59.7 \\
\bottomrule
\end{tabular}%
}
\end{table*}
