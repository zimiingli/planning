...
\begin{figure}[t]
	...
	\caption{\textbf{Same signal, opposite meaning.} Spearman
		$\rho$(token entropy, optimizer utility) across six
		environments on Qwen3-4B. Triggering on high entropy
		\emph{helps} on APPS and WebShop ($\rho{>}0$) but \emph{harms}
		on HotpotQA, TWExpress, and FEVER ($\rho{<}0$).}
...
We answer this through systematic measurement across six agent environments spanning QA~\cite{yang2018hotpotqa}, code generation~\cite{hendrycks2021apps}, web navigation~\cite{yao2022webshop}, fact verification~\cite{thorne2018fever}, text games~\cite{jansen2023twexpress}, and crafting planning~\cite{dagan2024plancraft}, using three model backbones. The same signal that predicts rollout benefit in code generation predicts rollout harm in fact verification, and can further reverse across model backbones within the same environment. Moreover, the most informative signal itself varies across environments, with structural features dominating in some settings while entropy carries little or no information in others.
