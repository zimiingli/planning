...
We answer this through systematic measurement across eight agent environments spanning QA~\cite{yang2018hotpotqa}, code
generation~\cite{hendrycks2021apps}, web navigation~\cite{yao2022webshop}, fact verification~\cite{thorne2018fever}, and text games~\cite{jansen2023twexpress}, using three model backbones. The signal--utility relationship reverses direction across environments:
...
Motivated by this analysis, we propose \textbf{EAAG} (\textbf{E}nvironment-\textbf{A}ware \textbf{A}daptive \textbf{G}ating), which learns environment-specific signal--utility relationships from interaction data. Since the primary challenge is identifying signal \emph{direction} rather than modeling capacity, EAAG invests its complexity in a discovery pipeline (randomized \emph{exploration}, LLM-based \emph{reasoning}, and sparse \emph{learning}) that feeds a lightweight gate adding zero per-step cost at deployment. Across eight environments and three LLM backbones, EAAG outperforms all fixed-direction baselines.
...
\item \textbf{Adaptive gating method.} We propose EAAG, which learns signal direction per environment instead of assuming it, outperforming all fixed-direction baselines across eight environments with zero deployment overhead.
\end{itemize}
